...
function notPasswuertResetGuyPutz() {
  ...
  const sheet = getLoginSheet();
  const werte = sheet.getDataRange().getValues();
  // Casse-insensitiv Sich, genau wéi an login() — sou triff dëst ëmmer déi
  // selwecht Zeil, déi och beim Umellen benotzt gëtt (och wann d'Schreifweis
  // an der Sheet vun "Guy Putz" ofwäicht, z.B. "Guy PUTZ").
  for (let i = 1; i < werte.length; i++) {
    if (String(werte[i][0] || "").trim().toLowerCase() === "guy putz") {
      const salt = Utilities.getUuid();
      sheet.getRange(i + 1, 4, 1, 2).setValues([[hashPin(NEIE_PIN, salt), salt]]);
      Logger.log("✅ PIN fir '" + werte[i][0] + "' zréckgesat op: " + NEIE_PIN);
      try {
        const ss = SpreadsheetApp.openById(OVERVIEW_SHEET_ID);
        let debugSheet = ss.getSheetByName("Debug");
        if (!debugSheet) {
        ...
        debugSheet.appendRow([
          Utilities.formatDate(new Date(), "Europe/Luxembourg", "dd.MM.yyyy HH:mm:ss"),
          "notPasswuertResetGuyPutz",
          "Neie PIN fir '" + werte[i][0] + "' (Zeil " + (i + 1) + "): " + NEIE_PIN,
          "",
        ]);
      } catch (debugErr) { }
      return;
